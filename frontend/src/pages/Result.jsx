...
const Result = () => {
  ...
  if (!result) {
    return (
      <Layout title="알림">
        <Container>
          <p style={{marginTop: '50px', textAlign: 'center'}}>진단 결과를 불러올 수 없습니다.</p>
    ...
    <Layout title="나의 건강 신호등" showBack={false}>
      <Container>
        ...
        <ReportCard>
          ...
          <QRWrapper>
            <QRCodeCanvas value={`${window.location.origin}/doctor-view/${result.id}`} size={160} />
            <p>담당 의사 확인용 QR 코드</p>
          </QRWrapper>
          ...
          <InfoGrid>
            <InfoItem>
              ...
              {/* 🔴 백엔드 필드명인 bodyPartKorean을 사용함 */}
              <div className="value">{result.bodyPartKorean || "미지정"}</div>
            </InfoItem>
            <InfoItem>
          ...
          <div style={{ background: '#f1f5f9', padding: '15px', borderRadius: '15px', fontSize: '14px', lineHeight: '1.6' }}>
        ...
        <div style={{ marginTop: '30px', width: '100%', display: 'flex', gap: '12px', maxWidth: '480px' }}>
          <BottomButton style={{ flex: 1, backgroundColor: '#CBD5E0' }} onClick={() => navigate('/')}>다시 하기</BottomButton>
          <BottomButton style={{ flex: 2 }} onClick={() => window.print()}>리포트 저장</BottomButton>
        </div>
      </Container>
    </Layout>
  );
